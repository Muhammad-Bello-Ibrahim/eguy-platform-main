...
export async function POST(request: NextRequest) {
  try {
    ...
    // Deduct from wallet. Note: this happens after the water bill was
    // already paid via SubAndGain, so a failed deduction here (very rare —
    ...
    if (!debit.success) {
      console.error("Water payment delivered but wallet debit failed (insufficient balance at debit time):", { userId: session.user.id, amount });
    }
