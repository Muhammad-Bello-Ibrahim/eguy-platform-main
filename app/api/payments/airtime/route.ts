...
export async function POST(request: NextRequest) {
  try {
    ...
    // Deduct from wallet. Note: this happens after the airtime was already
    // delivered by SubAndGain, so a failed deduction here (very rare —
    ...
    if (!debit.success) {
      console.error("Airtime purchase delivered but wallet debit failed (insufficient balance at debit time):", { userId: (session.user as any).id, amount });
    }
    // Record transaction
