...
export async function POST(request: NextRequest) {
  try {
    ...
    // Deduct from wallet. Note: this happens after the electricity token was
    ...
    if (!debit.success) {
      console.error("Electricity payment delivered but wallet debit failed (insufficient balance at debit time):", { userId: session.user.id, amount });
    }
    // Record transaction
