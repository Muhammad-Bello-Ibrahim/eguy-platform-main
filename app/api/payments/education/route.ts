...
export async function POST(request: NextRequest) {
  try {
    ...
    const { service, bills_code, regNumber, amount, pin, transactionPin } = await request.json()
    ...
    // Deduct from wallet. Note: this happens after the education pin was
    // already issued by SubAndGain, so a failed deduction here (very rare —
    // only if balance changed between the check above and now) can't be
    // undone by rejecting the request; we log it clearly so it surfaces in
    ...
    if (!debit.success) {
      console.error("Education pin delivered but wallet debit failed (insufficient balance at debit time):", { userId: session.user.id, amount });
    }
    // Record transaction
