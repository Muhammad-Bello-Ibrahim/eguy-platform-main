        ...
        return (
            <section className="mt-8">
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-4 px-4 pb-4">
                    {[1, 2, 3, 4].map((i) => (
                        <div key={i} className="min-w-[140px] bg-white dark:bg-card-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800/30 flex-shrink-0 space-y-3">
                            <Skeleton className="h-6 w-6 bg-slate-200 dark:bg-slate-800" />
    ...
    return (
        <section className="mt-8">
            <div className="flex items-center justify-between mb-5">
                ...
                <button className="text-primary text-xs font-bold uppercase tracking-widest hover:opacity-80 transition-opacity">See Stats</button>
            </div>
            <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-4 px-4 pb-4">
                {items.map((item, index) => (
                    <div
                        key={index}
                        className="min-w-[140px] bg-white dark:bg-card-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800/30 flex-shrink-0 shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                    >
                        <span className="material-icons-round text-primary mb-2 text-2xl block">{item.icon}</span>
                        <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mb-1 truncate">{item.title}</p>
